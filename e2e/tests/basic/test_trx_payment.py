"""
E2E Tests: TRX Payment

Tests for TRX (native token) payment flow using X402Client SDK.
"""
...
class TestTrxPayment:
    """TRX payment tests using X402Client SDK"""

    @pytest.mark.asyncio
    async def test_trx_payment_with_client_sdk(
        self,
        server_service,
        tron_client_signer,
    ):
        """Test TRX payment using X402Client SDK (like examples/python/client)"""
        # Setup X402 client
        ...
        async with httpx.AsyncClient(timeout=60.0) as http_client:
            ...
            response = await client.get(f"{server_service.base_url}/protected-trx")

            # Should succeed
            assert response.status_code == 200
            print(f"✅ TRX payment successful")
